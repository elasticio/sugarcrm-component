...
module.exports.getMetaModel = async function getMetaModel(configuration) {
  ...
  result.out.properties.records.properties = outSchema.out.properties;
...
function buildFilter(message, configuration) {
  const filter = [];
  ...
  function getOrItem(termIndex) {
    ...
    while (indx <= termNumber && (indx === termIndex || message.body[`link_${indx - 1}_${indx}`] !== 'OR')) {
      ...
      if (Object.keys(COMPARISON_OPERATORS_LIST).includes(sTerm.condition)) {
        ...
        value = `${sTerm.fieldValue}`;
      }

      orPart.push({
        [`${fieldNameSugarfy(sTerm.fieldName)}`]: {
...
module.exports.process = async function processAction(message, configuration) {
  this.logger.info(`Preparing to query ${configuration.module} objects...`);
  ...
  const filter = buildFilter(message, configuration);
  if (filter.length) {
  ...
  if (configuration.outputMethod === 'emitIndividually') {
    ...
    this.emit('data', messages.newMessageWithBody(response.records));
  }
};
