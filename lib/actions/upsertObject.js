...
exports.process = async function ProcessAction(msg, cfg) {
  ...
  try {
    if (cfg.utilizeAttachment) {
      ...
      if (key) {
        this.logger.info(`Attachment: ${key}`);
        ...
        this.logger.info(`Field for the attachment: ${fileField}`);
        ...
        if (fileField) {
          ...
          await instance.makeRequestStream(`${encodeURIComponent(cfg.module)}/${record.id}/file/${fileField}/?filename=${key}`, 'PUT', stream, null);
