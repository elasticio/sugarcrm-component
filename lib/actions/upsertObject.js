...
exports.process = async function ProcessAction(msg, cfg) {
  ...
  if (msg.body.id) {
    ...
    this.logger.trace(`Will perform update to ${module} with id ${id}`);
    ...
    this.logger.trace(`Will create a ${module}`);
  ...
  try {
    if (cfg.utilizeAttachment) {
      ...
      if (key) {
        ...
        this.logger.trace(`Attachment: ${key}`);
        ...
        this.logger.debug(`Field for the attachment: ${fileField}`);
