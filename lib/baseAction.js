...
function Action(componentObject, cfg, module) {
    ...
    this.createObject = async function createObject(msg) {
        ...
        console.log(`Created ${module} id: ${createdRecord.id}`);
    ...
    async function findObjectWithExternalId(externalId, externalIdProperty) {
        ...
        const results = await instance.makeRequest(`${encodeURIComponent(module)}/filter`, 'POST', query);
    ...
    async function doUpdate(idToUpdate, msg) {
        const updatedRecord
            = await instance.makeRequest(`${encodeURIComponent(module)}/${idToUpdate}`, 'PUT', msg.body);
        console.log('Updated Object Id: ' + updatedRecord.id);
    ...
    this.updateObject = async function updateObject(msg, externalIdProperty) {
        ...
        delete msg.body.id;
        console.log(`Attempting to update ${module} with external id ${externalId}`);
        const idToUpdate = await findObjectWithExternalId(externalId, externalIdProperty);
