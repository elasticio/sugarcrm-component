...
module.exports = function Service(config, emitter) {
  ...
  this.buildInSchemaForModule = async function buildInSchemaForModule(module) {
    ...
    const writableKeys = Object.keys(moduleFields)
      ...
      // Ignore concatenated products
      // Ignore calculated properties
      // Ignore
      // Ignore related properties (i.e. properties which are columns from a joined table)
      // Ignore linked fields for the moment
      .filter((key) => key !== '_hash'
          && !moduleFields[key].readonly
          && !moduleFields[key].fields
          && !moduleFields[key].calculated
          && moduleFields[key].type !== 'relate'
          && moduleFields[key].type !== 'link'
          && (moduleFields[key].source !== 'non-db' || ['date', 'url', 'id', 'bool', 'enum', 'int', 'text', 'varchar'].includes(moduleFields[key].type)));
