...
function Service (config, emitter) {
    ...
    const fetchAndSaveToken = async function fetchAndSaveToken(authParameters) {
        ...
        cfg.auth = authTokenResponse.body;
        cfg.auth.access_token_expiry = new Date(now.getTime() + cfg.auth.expires_in);
        cfg.auth.refresh_token_expiry = new Date(now.getTime() + cfg.auth.refresh_expires_in);
