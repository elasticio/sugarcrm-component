...
function Service (config, emitter) {
    ...
    const auth = async function() {
        ...
        if (now < cfg.oauth.access_token_expiry) {
        ...
        console.log(`The current access token has expired at ${cfg.oauth.access_token_expiry.toISOString()}`);
        ...
        console.log(`The current refresh token has also expired at ${cfg.oauth.refresh_token_expiry.toISOString()}.  Attempting to fetch tokens with password.`);
        return await getTokenUsingUserCredentials();
    };
