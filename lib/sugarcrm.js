...
module.exports = function Service(config, emitter) {
   ...
   * @returns {boolean}
   */
  function wasAuthenticationSuccessful(sugarCrmResponse) {
    if (sugarCrmResponse.statusCode >= 200 && sugarCrmResponse.statusCode < 300) {
      return true;
    }

    if (sugarCrmResponse.statusCode === 401 && sugarCrmResponse.body
      && sugarCrmResponse.body.error === 'invalid_grant') {
      return false;
    }

    throw new Error(`Unexpected response from provider.  Status code: ${sugarCrmResponse.statusCode} Body:
            ${JSON.stringify(sugarCrmResponse.body)}`);
  }
