...
function Service (config, emitter) {
    ...
    const fetchAndSaveToken = async function fetchAndSaveToken(authParameters) {
        ...
        console.log(`Successfully retrieved updated tokens.  Access token expires at ${cfg.oauth.access_token_expiry}.  Refresh token expires at ${cfg.oauth.refresh_token_expiry}`);
    ...
    const auth = async function() {
        ...
        if (now < new Date(cfg.refresh_token_expiry)) {
        ...
        console.log(`The current refresh token has also expired at ${cfg.oauth.refresh_token_expiry}.  Attempting to fetch tokens with password.`);
        return await getTokenUsingUserCredentials();
    };
     ...
     */
    this.getListOfModules = async function getListOfModules() {
        const moduleData = await makeRequest('metadata?type_filter=full_module_list', 'GET');
        return Object.keys(moduleData.full_module_list).sort();
    }
